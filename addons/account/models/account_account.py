...
class AccountAccount(models.Model):
    ...
    parent_id = fields.Many2one(
        ...
        domain="[('account_type', '=', account_type), '!', ('id', 'child_of', id)]",
        ondelete='set null',
    ...
    @api.constrains('parent_id', 'account_type', 'parent_path')
    def _check_account_and_parent_same_type(self):
        for record in self:
            if record.parent_id and record.parent_id.account_type != record.account_type:
                raise ValidationError(_("Account and parent must have the same account type"))
            if self.env['account.account'].search([('account_type', '!=', record.account_type), ('parent_path', '=like', record.parent_path + '%')], limit=1):
                raise ValidationError(_("Cannot change account type because it has children of the same type"))
    ...
    def _compute_field_path(self, fname):
        for record in self:
            record[f'{fname}_path'] = " / ".join(record.parent_ids.filtered(operator.itemgetter(fname)).mapped(fname))

    def _compute_sql_field_path(self, table, fname):
        query = table._query
        path_table_alias = f'{fname}_path_table'
        company_fname = table._with_model(self.with_company(self.env.company.root_id).sudo())[fname]
        query.add_join(
            'LEFT JOIN LATERAL',
            path_table_alias,
            SQL("""(
                      WITH account_account_intermediary AS (
                        SELECT %(id)s AS id,
                               STRING_TO_ARRAY(RTRIM(%(parent_path)s, '/'), '/') AS parent_path,
                               %(field_col_name)s AS %(fname)s
                          FROM account_account AS %(table_alias)s
                    )
                    SELECT child.id AS id,
                           STRING_AGG(ancestor.%(fname)s, ' / ' ORDER BY ord) AS result_path
                      FROM account_account_intermediary child
        CROSS JOIN LATERAL UNNEST(child.parent_path) WITH ORDINALITY AS t(id, ord)
                      JOIN account_account_intermediary ancestor ON ancestor.id = t.id::int
                  GROUP BY child.id
                )""",
                id=table.id,
                parent_path=table.parent_path,
                field_col_name=company_fname,
                fname=SQL(fname),
                table_alias=SQL(table._alias),
            ),
            SQL('%(alias)s.id = %(account_id)s', alias=SQL(path_table_alias), account_id=table.id),
        )
        path_table = TableSQL(path_table_alias, None, query)
        return SQL('%(path_table)s', path_table=path_table.result_path)
    ...
    def _search(self, domain, offset=0, limit=None, order=None, *, active_test=True, bypass_access=False):
        ...
        include_inactive_account_parents = (
            active_test and
            self.env.context.get('active_test', True) and
            self.env.context.get('include_inactive_account_parents', False) and
            not any(leaf.field_expr == self._active_name for leaf in Domain(domain).iter_conditions())
        )
    ...
    def _ensure_code_is_unique(self):
        """ Check account codes per companies.
            ...
            and both methods need to be kept in sync.
        """
        # Check 1: Check that no child or parent companies have an account with the same code.
        ...
        for company, account_ids in account_ids_to_check_by_company.items():
            ...
            # Check 1.1: Check that there are no duplicates in the given recordset.
            accounts_by_code = accounts.with_company(company).filtered(lambda account: account.code).grouped('code')
            ...
            # Check 1.2: Check that there are no duplicates in database
