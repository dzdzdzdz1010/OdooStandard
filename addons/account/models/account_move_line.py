...
class AccountMoveLine(models.Model):
    ...
    def _compute_cumulated_balance(self):
        ...
        This is limited to only the view with the specific ordering of `date desc, move_name desc, id` and a domain
        containing only one domain. As the cumulated balance only makes sense in that specific ordering.

        This is also very heavy to calculate as it can require to sum the entirity of the move lines on an account during
        the entire life of a database.
        """
        def _get_domain_information(domain):
            ...
            for domain_element in domain:
                ...
                elif isinstance(domain_element, str) and domain_element == '&':
                    continue
        ...
        if order != 'date desc, move_name desc, id' or account_id is None or not self:
            self.cumulated_balance = 0
            return

        account = self.env['account.account'].browse(account_id)
        if len(set(account.company_ids.mapped('currency_id'))) != 1:
            # If the account is shared between multiple currencies, we dont want to handle converting the balance
            self.cumulated_balance = 0
            return
