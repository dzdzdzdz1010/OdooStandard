...
class TestAccountMove(AccountTestInvoicingCommon):
    ...
    def test_post_invoice_fails_with_account_and_journal_company_inconsistency(self):
        ...
        move = self.env['account.move'].create({
            'line_ids': [
                Command.create({'name': 'debit_line', 'debit': 100.0, 'account_id': self.company_data['default_account_revenue'].id}),
                Command.create({'name': 'credit_line', 'credit': 100.0, 'account_id': self.company_data['default_account_revenue'].id}),
            ]
        })
