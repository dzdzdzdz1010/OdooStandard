...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_lines(self, record, tree, xpath, document_type=False, tax_type=False, qty_factor=1):
        logs = []
        lines_values = []
        vehicle = self._import_vehicle(tree, 'move', document_type)
        for line_tree in tree.iterfind(xpath):
            ...
            if self._need_vehicle_id(document_type):
                line_values['vehicle_id'] = vehicle if vehicle else self._import_vehicle(line_tree, 'line', document_type)
    ...
    def _import_vehicle(self, tree, tree_type, document_type):
        """
        ...
        """
        def default_condition(node, value):
            return node.findtext('./{*}Name') == value
        if self._need_vehicle_id(document_type):
            ...
            paths = [
                ...
                #   'condition': lambda node, value: 'where node = parent node, value = vin_identifier',
                ...
                {'path_type': 'line', 'vin_identifier': 'VIN'},  # BMW
                {
                    ...
                    'condition': lambda node, value: True,
                    'value_path': './{*}ID',
                },
            ]
            ...
            for path in [p for p in paths if p.get('path_type') == tree_type]:
                nodes = tree.findall(path.get('parent_node_path', default_parent_node_path))
                for node in nodes:
                    if path.get('condition', default_condition)(node, path.get('vin_identifier', '')):
                        vin_sn = node.findtext(path.get('value_path', default_value_path))
    ...
    def _need_vehicle_id(self, document_type):
        return document_type == 'in_invoice' and self.env.get('fleet.vehicle') is not None
