...
class AccountEdiUBL(models.AbstractModel):
    ...
    def _import_attachments(self, invoice, tree):
        if additional_docs := super()._import_attachments(invoice, tree):
            return additional_docs

        if invoice.move_type != 'in_invoice':
            return []

        # if an embedded PDF isn't present in the XML, we attempt to create one.
        try:
            pdf_raw, _ = self.env['ir.actions.report'].with_context(
                            force_report_rendering=True,
                            report_layout='web.external_layout_standard'  # forcing the "light" layout
                        )._render_qweb_pdf('account.account_invoices', res_ids=[invoice.id])

            name = (invoice.display_name or 'Bill').replace('Draft', '') + ' - Generated by Odoo from the XML file.pdf'

            # add a watermark to the generated pdf
            pdf_stream = io.BytesIO(pdf_raw)
            pdf_stream = pdf.add_banner(pdf_stream, 'Generated by Odoo', logo=False)
            pdf_raw = pdf_stream.getvalue()

            attachment = self.env['ir.attachment'].create({
                "name": name,
                "res_id": invoice.id,
                "res_model": "account.move",
                "datas": base64.b64encode(pdf_raw),
                "type": "binary",
