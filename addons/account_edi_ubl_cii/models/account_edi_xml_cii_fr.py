...
def _find_or_create(parent, tag, after=None, before=None):
    """Find an element or create it if not found."""
    ns_tag = tag
    if ':' in tag:
        prefix, local = tag.split(':', 1)
        ns = CII_NAMESPACES.get(prefix)
        if ns:
            ns_tag = '{%s}%s' % (ns, local)

    elem = parent.find(ns_tag, CII_NAMESPACES)
    if elem is None:
        elem = _make_elem(tag)
        if after is not None:
            after_elem = parent.find(after.replace(':', '/').replace('/', ':'), CII_NAMESPACES) if ':' in after else parent.find(after)
            if after_elem is not None:
                idx = list(parent).index(after_elem)
                parent.insert(idx + 1, elem)
            else:
                parent.append(elem)
        elif before is not None:
            before_elem = parent.find(before.replace(':', '/').replace('/', ':'), CII_NAMESPACES) if ':' in before else parent.find(before)
            if before_elem is not None:
                idx = list(parent).index(before_elem)
...
class AccountEdiXmlCiiFr(models.AbstractModel):
    ...
    """
    France CIUS (Core Invoice Usage Specification) implementation for CII.
    Adds French-specific elements by post-processing the base CII XML.

    Key additions:
    - SubjectCode in IncludedNote (BT-21)
    - CountrySubDivisionName in addresses (BT-39, BT-54, BT-68, BT-79)
    - TradingBusinessName in SpecifiedLegalOrganization (BT-28, BT-45)
    - Description (Legal Form) in trade parties (BT-33)
    - DepartmentName in DefinedTradeContact (BT-41-0, BT-56-0)
    - LineThree in PostalTradeAddress (BT-162, BT-163, BT-164, BT-165)
    - Tax Representative Party (BG-11)
    - Payee Trade Party (BG-10)
    - Additional Referenced Documents (BG-24)
    """
    ...
    def _export_invoice(self, invoice):
        ...
        This method:
        1. Calls the parent to generate base CII XML
        2. Post-processes the XML to add French-specific elements
        3. Returns the modified XML
        """
    ...
    def _add_electronic_address(self, party_elem, partner):
        ...
        Priority: peppol_endpoint > SIRET > company_registry > VAT > email
        """
        ...
        # Priority 1: Peppol endpoint (standard Odoo field)
        if commercial_partner.peppol_endpoint:
            uri_id = _make_elem('ram:URIID', commercial_partner.peppol_endpoint)
            scheme_id = commercial_partner.peppol_eas or '0009'
        # Priority 2: SIRET (French company identifier)
        elif commercial_partner.siret:
            uri_id = _make_elem('ram:URIID', commercial_partner.siret)
            scheme_id = '0009'  # SIRET scheme
        # Priority 3: Company registry
        elif commercial_partner.company_registry:
            uri_id = _make_elem('ram:URIID', commercial_partner.company_registry)
            scheme_id = '0009'  # SIRET/SIREN scheme
        # Priority 4: VAT number
        elif commercial_partner.vat:
            uri_id = _make_elem('ram:URIID', commercial_partner.vat)
            # Use country-specific VAT scheme
            country_code = commercial_partner.country_id.code if commercial_partner.country_id else 'FR'
            vat_schemes = {
                'FR': '9957',  # France VAT
                'BE': '9925',  # Belgian VAT
                'DE': '9930',  # Germany VAT
            }
            scheme_id = vat_schemes.get(country_code, '9957')
        # Priority 5: Email as fallback
        elif commercial_partner.email:
            uri_id = _make_elem('ram:URIID', commercial_partner.email)
            scheme_id = 'EM'  # Email scheme
        # Priority 6: Generate from partner ref/name (absolute fallback - BT-34/BT-49 are mandatory)
        else:
            # Use partner reference, or name as absolute fallback
            fallback_id = commercial_partner.ref or commercial_partner.name or f"PARTNER-{commercial_partner.id}"
            uri_id = _make_elem('ram:URIID', fallback_id)
            scheme_id = '0002'  # SIRENE scheme as generic fallback

        if uri_id is not None and scheme_id:
            uri_id.set('schemeID', scheme_id)
    ...
    def _import_french_fields(self, invoice, tree):
        ...
        if siren:
            ...
            # Try to update partner's company_registry if empty
            partner = invoice.company_id.partner_id.commercial_partner_id
            if partner and not partner.company_registry:
                partner.company_registry = siren
    ...
    def _import_retrieve_partner_vals(self, tree, role):
        ...
        if uri_info:
            vals['peppol_endpoint'] = uri_info.get('uri_id')
            vals['peppol_eas'] = uri_info.get('scheme_id')
