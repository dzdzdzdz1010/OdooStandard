...
class AccountEdiXmlCiiFrExtended(models.AbstractModel):
    ...
    - Business Process specification (BT-23)
    - Agent parties (Sales Agent, Buyer Agent, Invoicer, Invoicee, Payer)
    - Extended line-level references and delivery information
    - Contract reference type code (BT-X-405)
    - Invoice reference type code (BT-X-555)
    """
    ...
    def _get_agent_party_vals(self, invoice, agent_type):
        """Get agent trade party values for various agent types.

        agent_type: 'sales_agent', 'buyer_agent', 'invoicer', 'invoicee', 'payer'
        """
    ...
    def _build_agent_party_vals(self, agent, agent_type):
        ...
        if agent.company_registry or siret:
            vals['legal_organization'] = {
                'id': siret or agent.company_registry,
                'scheme_id': '0002',
        ...
        if 'l10n_fr_uri_id' in agent._fields and agent.l10n_fr_uri_id:
            ...
            vals['uri_scheme_id'] = (
                agent.l10n_fr_uri_scheme_id or None
