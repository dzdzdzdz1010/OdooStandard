...
# French-specific note subject codes (EN16931)
# These codes are used in BT-21 (Invoice note subject code)
FR_NOTE_SUBJECT_CODES = {
...
class AccountEdiXmlUblFr(models.AbstractModel):
    ...
    def _get_french_billing_mode(self, invoice):
        """Get the French billing mode (BT-23 / ProfileID) based on customer type.

        [BR-FR-08] ProfileID must be one of: B1, S1, M1, B2, S2, M2, B4, S4, M4, S5, S6, B7, S7
        - B = B2B (private business)
        - S = B2G (public sector)
        - M = Mixed
        - 1 = Domestic
        - 2 = Intracom
    ...
    def _get_siren_from_partner(self, partner):
        ...
        # Priority 2: Check peppol_endpoint with EAS 0009 (SIRET) or 0002 (SIREN/SIRENE)
        if 'peppol_endpoint' in commercial_partner._fields and commercial_partner.peppol_endpoint:
            endpoint = commercial_partner.peppol_endpoint.strip()
            eas = commercial_partner.peppol_eas or '' if 'peppol_eas' in commercial_partner._fields else ''
            if eas == '0009' and len(endpoint) == 14 and endpoint.isdigit():
                return endpoint[:9]  # SIRET → SIREN
            elif eas == '0002' and len(endpoint) == 9 and endpoint.isdigit():
                return endpoint  # Already SIREN

        # Priority 3: Check company_registry
    ...
    def _add_invoice_payee_party_nodes(self, document_node, vals):
        ...
        document_node['cac:PayeeParty'] = {
            ...
            'cac:PartyIdentification': {
                'cbc:ID': {
                    '_text': payee.ref or payee.vat,
                    'schemeID': '0002' if payee.company_registry else None
                }
            } if payee.ref or payee.vat else None,
