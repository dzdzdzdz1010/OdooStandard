...
class AccountEdiXmlUblFrExtended(models.AbstractModel):
    ...
    """
    Extended profile for French e-invoicing that adds:
    - Agent parties (Sales Agent, Buyer Agent, Invoicer, Invoicee, Payer)
    - ServiceProviderParty
    - Extended document references with DocumentType
    - Extended line references (BillingReference, DespatchLineReference, ReceiptLineReference)
    - Extended order references (OrderReference with SalesOrderID)
    """
    ...
    def _get_agent_party_node(self, agent):
        ...
        peppol_endpoint = agent.peppol_endpoint if 'peppol_endpoint' in agent._fields else None
        peppol_eas = agent.peppol_eas if 'peppol_eas' in agent._fields else None
        return {
            'cac:PartyLegalEntity': {
                'cbc:RegistrationName': {'_text': agent.name},
                'cbc:CompanyID': {
                    '_text': agent.company_registry or agent.vat,
                    'schemeID': '0002' if agent.company_registry else '0223'
                } if agent.company_registry or agent.vat else None,
            },
