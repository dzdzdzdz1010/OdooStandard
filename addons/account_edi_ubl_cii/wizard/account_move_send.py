...
class AccountMoveSend(models.TransientModel):
    ...
    def _need_invoice_document(self, invoice):
        ...
        invoice_ams = self.env['account.move.send'].search([('move_ids', 'any', [('id', '=', invoice.id)])])
        if invoice_ams and invoice_ams[-1].enable_ubl_cii_xml and invoice_ams[-1].checkbox_ubl_cii_xml and invoice._need_ubl_cii_xml():
            return True
        return result
