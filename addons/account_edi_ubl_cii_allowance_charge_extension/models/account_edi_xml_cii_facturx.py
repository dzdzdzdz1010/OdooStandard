    ...
    def _add_invoice_line_allowance_charge_vals(self, template_values):
        ...
        for line_vals in template_values['invoice_line_vals_list']:
            ...
            for grouping_key, tax_detail in tax_details_per_record.items():
                ...
                # Fixed taxes mapped as charges
                if (
                    grouping_key.get('amount_type') == 'fixed'
                    and not allowance_charge_vals.get('allowance_charge_reason_code', False)
                ):
                    line_vals['allowance_charge_vals_list'].append({
                        'indicator': 'true',
                        'reason': tax_detail.get('tax_name'),
                        'reason_code': 'AEO',
                        'amount': tax_detail.get('tax_amount_currency'),
                    })

                # Explicit allowance/charge defined on tax category
                elif 'charge_indicator' in allowance_charge_vals:
                    line_vals['allowance_charge_vals_list'].append({
                        ...
                        'calculation_percent': grouping_key.get('amount'),
                        # Only keep basis_amount in conjunction with calculation_percent
                        'basis_amount': tax_detail.get('base_amount_currency') if grouping_key.get('amount') else None,
