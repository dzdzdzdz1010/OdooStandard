...
class TestPeppolMessage(TestAccountMoveSendCommon, MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.private_key = cls.env['certificate.key'].create({
            ...
            'content': BinaryFile(f'{FILE_PATH}/private_key.pem'),
        })
        ...
        if url == '/api/peppol/1/get_document':
            ...
            if uuid == FAKE_UUID[0]:
                ...
                response_content = {
                    ...
                    'enc_key': BinaryFile(f'{FILE_PATH}/enc_key').decode(),
                    'document': BinaryFile(f'{FILE_PATH}/{cls.mocked_incoming_invoice_fname}').decode(),
                    'state': 'done' if not cls.env.context.get('error') else 'error',
                    'direction': 'incoming',
                    'document_type': 'Invoice',
                }
