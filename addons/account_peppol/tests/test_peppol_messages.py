...
class TestPeppolMessage(TestAccountMoveSendCommon, MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.private_key = cls.env['certificate.key'].create({
            ...
            'content': BinaryValue.from_file(f'{FILE_PATH}/private_key.pem'),
        })
        ...
        if url == '/api/peppol/1/get_document':
            ...
            if uuid == FAKE_UUID[0]:
                ...
                response_content = {
                    ...
                    'enc_key': BinaryValue.from_file(f'{FILE_PATH}/enc_key').decode(),
                    'document': BinaryValue.from_file(f'{FILE_PATH}/{cls.mocked_incoming_invoice_fname}').decode(),
                    'state': 'done' if not cls.env.context.get('error') else 'error',
                    'direction': 'incoming',
                    'document_type': 'Invoice',
                }
