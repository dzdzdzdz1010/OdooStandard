...
/**
 * Adding the current user activities in the Attendee Calendar day, week and month views.
 * Done using a patch to prevent loading this feature and its mail components in POS.
 */
...
patch(AttendeeCalendarCommonRenderer.prototype, {
    ...
    /**
     * @override
     * Disable edition of activity events (no drag & drop, no time extension, ...)
     */
    ...
    /**
     * @override
     * Adding activity events in the displayed Full Calendar events.
     */
    mapRecordsToEvents() {
    ...
    onEventContent(arg) {
        const { event } = arg;
        if (event.extendedProps.isActivity) {
            const activityEvent = this.props.model.data.activities[event.id];
            if (activityEvent) {
