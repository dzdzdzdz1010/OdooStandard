...
from odoo.tools import BinaryFile, BinaryValue
...
class TestKeysCertificates(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_key_1 = cls.env['certificate.key'].create({
            'name': 'Test key',
            'content': BinaryValue.from_bytes(private_key.private_bytes(
        ...
        cls.test_key_2_public = cls.env['certificate.key'].create({
            'name': 'Test key',
            'content': BinaryValue.from_bytes(private_key.public_key().public_bytes(
                encoding=serialization.Encoding.PEM,
    ...
    def test_der_certificate(self):
        certificate = self.env['certificate.certificate'].create({
            ...
            'content': BinaryValue.from_bytes(self.certificate_1.public_bytes(encoding=serialization.Encoding.DER)),
    ...
    def test_pem_certificate(self):
        certificate = self.env['certificate.certificate'].create({
            ...
            'content': BinaryValue.from_bytes(self.certificate_1.public_bytes(encoding=serialization.Encoding.PEM)),
            'private_key_id': self.test_key_1.id,
        })
    ...
    def test_pfx_certificate(self):
        certificate = self.env['certificate.certificate'].create({
            ...
            'content': BinaryFile('certificate/tests/data/cert.pfx'),
        })
    ...
    def test_is_valid(self):
        ...
        expired_cert = self.env['certificate.certificate'].create({
            ...
            'content': BinaryValue.from_bytes(old_certificate.public_bytes(encoding=serialization.Encoding.PEM)),
            'private_key_id': self.test_key_1.id,
        })
