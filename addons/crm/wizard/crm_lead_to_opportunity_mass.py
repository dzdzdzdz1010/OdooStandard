...
class CrmLead2opportunityPartnerMass(models.TransientModel):
    ...
    ], string='Related Customer', default='create', readonly=False, required=True)
    ...
    # the duplicated_lead_copies_ids field stores all the found duplicates, to be displayed in the dialog
    duplicated_lead_copies_ids = fields.Many2many('crm.lead',
        'lead2opp_mass_dup_lead_copies_rel', 'wizard_id', 'lead_id',
    ...
    team_id = fields.Many2one(
        'crm.team', 'Sales Team', default=False,
    ...
    def _compute_lead_count_message(self):
        for convert in self:
            ...
            total_duplicate_count = len(convert.duplicated_lead_copies_ids)
            if total_duplicate_count == 1:
                convert.lead_count_message = _(
                    "Found %(total_duplicate_count)s potential duplicate with %(selected_duplicate_count)s of your "
                    "selected leads. Choose a deduplication option to clean your data before converting",
                    total_duplicate_count=total_duplicate_count, selected_duplicate_count=selected_duplicate_count)
            else:
                convert.lead_count_message = _(
                    "Found %(total_duplicate_count)s potential duplicates with %(selected_duplicate_count)s of your "
                    "selected leads. Choose a deduplication option to clean your data before converting",
                    total_duplicate_count=total_duplicate_count, selected_duplicate_count=selected_duplicate_count)
    ...
    def _compute_duplicated_lead_ids(self):
        for convert in self:
            ...
            convert.duplicated_lead_copies_ids = all_duplicates.ids
    ...
    def _action_deduplicate(self):
        ...
        # What should deduplicate return? Just close the dialog with a success toast?
