...
class CrmMergeOpportunity(models.TransientModel):
    ...
    def action_merge(self):
        destination_lead_id = self._action_merge_to_opportunity()
        if not destination_lead_id:
            return {
    ...
    def _action_merge_to_opportunity(self):
        """Internal method that returns the id of the resulting opportunity. Used for testing."""
        self.ensure_one()
        return self.opportunity_ids.merge_opportunity(self.user_id.id, self.team_id.id).id
