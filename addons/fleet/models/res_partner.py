    ...
    def action_open_partner_cars(self):
        ...
        return {
            ...
            "views": [[self.env.ref("hr_fleet.fleet_vehicle_assignation_log_employee_view_list").id, "list"],
                      [False, "form"]],
            "domain": [("driver_id", "in", self.ids), ("driver_employee_id", "in", self.employee_ids.ids)],
            "context": dict(self.env.context, default_driver_id=self.id, default_driver_employee_id=self.employee_ids[0].id),
