...
class HrAttendance(models.Model):
    ...
    # split the attendance shift at the employee local midnight and save in UTC
        ...
        for company in all_companies:
            ...
            for att in to_verify_company:
                ...
                if (current_attendance_duration + previous_attendances_duration - max_tol) > expected_worked_hours:
                    estimated_checkout = att.check_in.replace(hour=23, minute=59, second=59)
                    att.check_out = estimated_checkout

                    # Latest Attendance with the estimated check_out
                    att = self.env['hr.attendance'].search([
                        ('employee_id', '=', att.employee_id.id),
                        ('check_out', '=', estimated_checkout),
                    ], limit=1)
                    first_check_in = att.check_in
                    total_worked_hours = att.worked_hours

                    # Check if there's a Previous Cross Day shift Attendance
                    previous_attendnace = self.env['hr.attendance'].search([
                        ('employee_id', '=', att.employee_id.id),
                        ('check_out', '=', att.check_in),
                    ], limit=1)
                    if previous_attendnace:
                        total_worked_hours += previous_attendnace.worked_hours
                        first_check_in = previous_attendnace.check_in

                    excess_hours = total_worked_hours - (expected_worked_hours + max_tol - previous_attendances_duration)
                    check_out = max(att.check_out - relativedelta(hours=excess_hours), first_check_in + relativedelta(seconds=1))
                    if check_out >= att.check_in:   # Adjustment affects to current record
                        att.write({
                            "check_out": check_out,
                            "out_mode": "auto_check_out"
                        })
                    else:   # Adjustment affects to previous record
                        if previous_attendnace:
                            previous_attendnace.write({
                                "check_out": max(previous_attendnace.check_out - relativedelta(hours=excess_hours - att.worked_hours),
                                                    previous_attendnace.check_in + relativedelta(seconds=1)),
                                "out_mode": "auto_check_out"
                            })
                        else:   # make current record minimally
                            att.write({
                                "check_out": att.check_in + relativedelta(seconds=1),
                                "out_mode": "auto_check_out"
                            })
                    att.message_post(
