...
class HrExpense(models.Model):
    ...
    def _send_submitted_expenses_mail(self):
        ...
        other_expenses_map = {
            ...
            for company, manager, expenses, max_date in self.sudo()._read_group(
                ...
                aggregates=['id:recordset', 'submission_date:max'],
            )
        }
        for company, expenses_submitted_per_company in self.grouped('company_id').items():
            ...
            for manager, expenses_submitted in expenses_submitted_per_company.grouped('manager_id').items():
                if not manager:
                    continue
                last_submission_date = other_expenses_map.get((company.id, manager.id), {}).get('max_submission_date')
                if last_submission_date and last_submission_date > (fields.Datetime.now() - timedelta(hours=12)):
                    continue

                all_submitted_expenses = other_expenses_map.get((company.id, manager.id), {}).get('expenses', self.env['hr.expense'])
    ...
    def action_submit(self):
        ...
        self.sudo().submission_date = fields.Datetime().now()
    ...
    def _do_reset_approval(self):
        self.sudo().write({'approval_state': False, 'approval_date': False, 'submission_date': False, 'account_move_id': False})
        self.update_activities_and_mails()
