# Part of Odoo. See LICENSE file for full copyright and licensing details.

from freezegun import freeze_time
from datetime import date

from odoo import Command
from odoo.tests import HttpCase
from odoo.tests.common import tagged
from odoo.addons.hr.tests.test_utils import get_admin_employee


@tagged('post_install', '-at_install')
class TestHrWorkEntryTypeTour(HttpCase):
    @freeze_time('01/17/2022')
    def test_hr_work_entry_type_tour(self):
        """
        Test Time Off multi company rule defined in hr_holidays_security for hr_work_entry_type.
        The available leave types are the ones whose:
            - Company is one of the selected companies.
            - Company is false but whose country is one the countries of the selected companies.
            - Company is false and country is false

        Define:
            - 2 Companies: company_1 and company_2
            - 3 Leave Types:
                * work_entry_type_1 whose country is set to the country of company_1.
                * work_entry_type_2 whose company is set to company_2.
                * work_entry_type_3 whose country and company are both False.

        work_entry_type_1 will be available if company_1 is one of the selected companies.
        work_entry_type_2 will be available if company_2 is one of the selected companies.
        work_entry_type_3 will always be available.
        """
        admin_user = self.env.ref('base.user_admin')
        admin_user.write({
            'email': 'mitchell.admin@example.com',
        })
        admin_employee = get_admin_employee(self.env)
        HRLeave = self.env['hr.leave']
        date_from = date(2022, 1, 17)
        date_to = date(2022, 1, 18)
        leaves_on_freeze_date = HRLeave.search([
            ('date_from', '>=', date_from),
            ('date_to', "<=", date_to),
            ('employee_id', '=', admin_employee.id)
        ])
        leaves_on_freeze_date.sudo().unlink()
        company_1 = self.env.company
        company_1.name = 'company_1'
        company_2 = self.env['res.company'].create({'name': 'company_2'})
        self.env["res.users"].browse(2).write({
            "company_ids": [Command.clear(), Command.link(company_1.id), Command.link(company_2.id)]
        })

        HrWorkEntryType = self.env['hr.work.entry.type'].with_user(admin_user)

        HrWorkEntryType.create({
            'name': 'work_entry_type_1',
            'code': 'work_entry_type_1',
            'requires_allocation': False,
            'leave_validation_type': 'hr',
            'country_id': company_1.country_id.id,
            'request_unit': 'day',
            'unit_of_measure': 'day',
        })
        HrWorkEntryType.create({
            'name': 'work_entry_type_2',
            'code': 'work_entry_type_2',
            'requires_allocation': False,
            'leave_validation_type': 'hr',
            'company_id': company_2.id,
            'request_unit': 'day',
            'unit_of_measure': 'day',
        })
        HrWorkEntryType.create({
            'name': 'work_entry_type_3',
            'code': 'work_entry_type_3',
            'requires_allocation': False,
            'leave_validation_type': 'hr',
            'request_unit': 'day',
            'unit_of_measure': 'day',
        })
        self.start_tour('/web', 'hr_work_entry_type_tour', login="admin")
