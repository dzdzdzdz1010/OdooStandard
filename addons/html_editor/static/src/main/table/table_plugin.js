...
export class TablePlugin extends Plugin {
    ...
    addColumn(position, reference, columnsToAdd = 1) {
        ...
        for (let columnOffset = 0; columnOffset < columnsToAdd; columnOffset++) {
            ...
            for (let rowIndex = 0; rowIndex < table.rows.length; rowIndex++) {
                ...
                // When inserting multiple columns "after", update the reference so
                // columns are appended in order.
                if (position === "after" && cell === reference) {
                    reference = newCell;
                }
            }
    ...
    addRow(position, reference, rowsToAdd = 1) {
        ...
        for (let rowOffset = 0; rowOffset < rowsToAdd; rowOffset++) {
            ...
            // When inserting multiple rows "after", update the reference so
