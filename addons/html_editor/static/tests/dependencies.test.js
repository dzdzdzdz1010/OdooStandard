...
describe("Implicit plugin dependencies", () => {
    test("input as an implicit dependency", async () => {
        for (const P of [
            ...
            // input dependency through the "beforeinput_listeners" and
            // "input_listeners" resources. This dependency was added because the
    ...
    test("position as an implicit dependency", async () => {
        for (const P of [PowerButtonsPlugin, CollaborationSelectionPlugin]) {
            // position dependency through the "layout_geometry_change_listeners"
