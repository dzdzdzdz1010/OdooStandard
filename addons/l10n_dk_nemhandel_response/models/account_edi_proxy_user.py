...
class AccountEdiProxyClientUser(models.Model):
    ...
    def _nemhandel_send_response(self, reference_moves, status, note=False):
        self.ensure_one()
        reference_moves = reference_moves.filtered('nemhandel_message_uuid')
        ...
        try:
            ...
            log_message = Markup(self.env._("An error occurred with the Nemhandel proxy while responding to this invoice's expeditor.<br/>Response: %s - %s - %s", status, e.code, e.message))
            ...
            if response.get('error'):
                log_message = Markup(self.env._(
                    "An error occurred with the Nemhandel server while responding to this invoice's expeditor.<br/>Status: %s - %s",
