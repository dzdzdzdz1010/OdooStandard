...
from odoo.tools import BinaryValue, html_sanitize, zeep
...
class TestL10nEsEdiVerifactuCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.certificate = cls.env['certificate.certificate'].create({
            'content': BinaryValue.from_file(certificate_path),
    ...
    def _json_file_to_dict(self, json_file):
        json_string = BinaryValue.from_file(json_file).content
    ...
    def _mock_response(self, status_code, response_file, content_type='text/xml;charset=UTF-8'):
        ...
        response.text = BinaryValue.from_file(response_file).content
    ...
    def _mock_zeep_registration_operation(self, response_file_json, name=""):
        ...
        zeep_response_dict = json.loads(BinaryValue.from_file(response_file_json).content)
        if name:
