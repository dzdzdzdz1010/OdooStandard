...
class PdpFlowAggregator(models.AbstractModel):
    ...
    def _is_international_partner(self, move):
        """Return True when the move partner is in a different country than the company."""
        partner = move.commercial_partner_id
        company_country = move.company_id.country_id
        partner_country = partner.country_id
        if partner_country and company_country:
            return partner_country != company_country
    ...
    def _get_payment_moves(self, moves):
        ...
        for move in moves:
            ...
            for line in move.invoice_line_ids.filtered(lambda l: l.display_type == 'product'):
                taxes = line.tax_ids
                if any((tax.l10n_fr_pdp_tt81_category or '').strip() == 'TPS1' for tax in taxes):
