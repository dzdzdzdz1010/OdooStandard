...
class AccountMove(models.Model):
    ...
    l10n_hr_currency_rate = fields.Float(
        string='Currency Rate (HR)',
        compute='_compute_l10n_hr_currency_rate',
        digits=0,
    ...
    def _check_l10n_hr_process_type(self):
        for record in self:
            if record.country_code == 'HR' and (record.l10n_hr_process_type == 'P9') == (record.move_type != 'out_refund'):
                raise ValidationError(_('Business Process Type P9 can only be used with credit notes and vice versa.'))
    ...
    def _compute_l10n_hr_currency_rate(self):
        for move in self:
            if move.currency_id:
                move.l10n_hr_currency_rate = move.env['res.currency']._get_conversion_rate(
    ...
    def l10n_hr_edi_mer_action_reject(self):
        self.ensure_one()
        return {
            'name': _("Reject MojEracun invoice"),
    ...
    def l10n_hr_edi_mer_action_fetch_document_status_company(self):
        self.env.company._l10n_hr_mer_fetch_document_status_company(from_cron=False)

    def l10n_hr_edi_mer_action_fetch_new_documents(self):
        self.env.company._l10n_hr_mer_get_new_documents(undelivered_only=True)

    def l10n_hr_edi_mer_action_fetch_all_documents(self):
        self.env.company._l10n_hr_mer_get_new_documents(undelivered_only=False)
