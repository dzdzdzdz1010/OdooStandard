...
class HrLeave(models.Model):
    ...
    def _l10n_in_is_working(self, on_date, public_holiday_dates, resource_calendar):
        if self.holiday_status_id.l10n_in_sandwich_policy == "public_holiday":
            return on_date not in public_holiday_dates
        elif self.holiday_status_id.l10n_in_sandwich_policy == "weekend":
    ...
    def _l10n_in_apply_sandwich_rule(self, public_holidays_date_by_company, leaves_dates_by_employee):
        ...
        total_leaves = 0
        policy = self.holiday_status_id.l10n_in_sandwich_policy
        current_date = date_from
        while current_date <= date_to:
            is_weekend = not self.resource_calendar_id._works_on_date(current_date)
