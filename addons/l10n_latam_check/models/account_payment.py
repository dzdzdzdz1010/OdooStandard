...
class AccountPayment(models.Model):
    ...
    def _prepare_move_liquidity_line_vals(self, line_vals):
        ...
        if not l10n_latam_checks:
            return super()._prepare_move_liquidity_line_vals(line_vals)
        ...
        amounts_to_distribute = self.env['account.tax']._distribute_delta_amount_smoothly(
            precision_digits=self.currency_id.decimal_places,
            delta_amount=line_vals['amount'],
            target_factors=target_factors,
        )
        for target_factor, amount_to_distribute in zip(target_factors, amounts_to_distribute):
            target_factor['values']['amount_currency'] = amount_to_distribute

        amounts_to_distribute = self.env['account.tax']._distribute_delta_amount_smoothly(
            precision_digits=self.currency_id.decimal_places,
            delta_amount=line_vals['balance'],
            target_factors=target_factors,
        )
        for target_factor, amount_to_distribute in zip(target_factors, amounts_to_distribute):
            target_factor['values']['balance'] = amount_to_distribute

        return [target['values'] for target in target_factors]
