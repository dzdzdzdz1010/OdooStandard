...
class L10n_LatamPaymentRegisterCheck(models.TransientModel):
    ...
    bank_id = fields.Many2one('res.partner.bank', compute='_compute_bank_id', store=True, readonly=False)
    ...
    def _compute_bank_id(self):
        for rec in self:
            if rec.payment_register_id.payment_method_line_id.code != 'new_third_party_checks':
                rec.bank_id = False
                continue
            rec.bank_id = rec.payment_register_id.partner_id.bank_ids[:1].id
