...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_sa_post_zatca_edi(self, invoice):  # no batch ensure that there is only one invoice
        ...
        response_data, signed_xml = self._l10n_sa_export_zatca_invoice(invoice)
        ...
        if response_data.get('error'):
            ...
            invoice._l10n_sa_log_results(signed_xml, response_data, error=response_data.get('rejected'))
            return {
                invoice: {
                    **response_data,
                    'response': signed_xml
        ...
        cleared_xml = self._l10n_sa_postprocess_einvoice_submission(invoice, signed_xml, response_data)
