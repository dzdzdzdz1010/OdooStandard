...
class MailMessage(models.Model):
    ...
    * snailmail.letter (snailmail addon)
    ...
    is_bookmarked = fields.Boolean(
        'Bookmarked', compute='_compute_is_bookmarked', search='_search_is_bookmarked', compute_sudo=False,
        help='Current user has a bookmarked notification linked to this message')
    # tracking
    ...
    def _compute_is_bookmarked(self):
        ...
        # TDE FIXME: use SQL
        bookmarked = self.sudo().filtered(lambda msg: self.env.user.partner_id in msg.bookmarked_partner_ids)
        for message in self:
    ...
    def remove_all_bookmarks(self):
        """ Remove all bookmarks for the current partner. """
        bookmarked_messages = self.search([("bookmarked_partner_ids", "in", self.env.user.partner_id.id)])
        # sudo: mail.message - a user can remove bookmarks from messages they can read
        bookmarked_messages.sudo().bookmarked_partner_ids = [Command.unlink(self.env.user.partner_id.id)]
        if bookmarked_messages:
            store = Store(bus_channel=self.env.user)
            store.add(bookmarked_messages, ["is_bookmarked"])
            store.bus_send()

    def toggle_message_bookmark(self):
        """ Toggle messages as (un)bookmark. Technically, the notifications related
            to uid are set to (un)bookmarked.
        """
        self.ensure_one()
        self.check_access('read')
        bookmarked = not self.is_bookmarked
        if bookmarked:
            # sudo: mail.message - a user can bookmark a message they can read
            self.sudo().bookmarked_partner_ids = [Command.link(self.env.user.partner_id.id)]
        else:
            # sudo: mail.message - a user can unbookmark a message they can read
            self.sudo().bookmarked_partner_ids = [Command.unlink(self.env.user.partner_id.id)]
        store = Store(bus_channel=self.env.user)
        store.add(self, ["is_bookmarked"])
        store.bus_send()
