...
class CreateChannelDialog extends Component {
    ...
    setup() {
        ...
        this.state = useState({ name: this.props.name || "", isInvalid: false, readonly: false });
    }
    ...
    async onClickConfirm() {
        ...
        await makeNewChannel(name, this.store, readonly);
        this.props.close();
    }
}
 ...
 */
async function makeNewChannel(name, store, readonly = false) {
    const { channel } = await store.fetchStoreData(
        "/discuss/create_channel",
        { name, group_id: store.internalUserGroupId, readonly: readonly },
