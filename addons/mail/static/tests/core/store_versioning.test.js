...
const SINGLE_FIELD_CASES = [
    {
        ...
        steps: [
            {
                values: { name: "v1" },
                meta: { snapshot: { xmin: 10, xmax: 10, xip: [], current_xact_id: null } },
                expected: { name: "v1" },
            },
            {
                values: { name: "v3" },
                meta: { snapshot: { xmin: 40, xmax: 40, xip: [], current_xact_id: null } },
                expected: { name: "v3" },
                description: "V3 read's snapshot is newer.",
            },
            {
                values: { name: "v2" },
                meta: { snapshot: { xmin: 30, xmax: 30, xip: [], current_xact_id: null } },
                expected: { name: "v3" },
                description: "V2 read's snapshot is older.",
            },
            {
                values: { name: "v4" },
                meta: { snapshot: { xmin: 65, xmax: 68, xip: [65, 66], current_xact_id: null } },
                expected: { name: "v4" },
                description: "V4 read's snapshot is newer.",
            },
            {
                values: { name: "v5" },
                meta: { snapshot: { xmin: 65, xmax: 68, xip: [65], current_xact_id: null } },
                expected: { name: "v5" },
                description:
    ...
    },
    {
        ...
        steps: [
            {
                values: { name: "v3" },
                meta: { snapshot: { xmin: 30, xmax: 40, xip: [30, 35], current_xact_id: null } },
                expected: { name: "v3" },
            },
            {
                values: { name: "v2" },
                meta: {
                    snapshot: { xmin: 20, xmax: 20, xip: [], current_xact_id: 20 },
            ...
            },
            {
                values: { name: "v4" },
                meta: {
                    snapshot: { xmin: 20, xmax: 20, xip: [], current_xact_id: 35 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v4" },
            ...
            },
            {
                values: { name: "v6" },
                meta: {
                    snapshot: { xmin: 50, xmax: 50, xip: [], current_xact_id: 50 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v6" },
            ...
            {
                values: { name: "v7" },
                meta: { snapshot: { xmin: 60, xmax: 60, xip: [], current_xact_id: null } },
                expected: { name: "v7" },
            ...
            },
            {
                values: { name: "v8" },
                meta: {
                    snapshot: { xmin: 60, xmax: 60, xip: [], current_xact_id: 60 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v8" },
        ...
        steps: [
            {
                values: { name: "v1" },
                meta: {
                    snapshot: { xmin: 10, xmax: 10, xip: [], current_xact_id: 10 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v1" },
            },
            {
                values: { name: "v3" },
                meta: {
                    snapshot: { xmin: 30, xmax: 30, xip: [], current_xact_id: 30 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v3" },
    ...
    },
    {
        ...
        steps: [
            ...
            },
            {
                ...
                meta: { snapshot: { xmin: 20, xmax: 20, xip: [], current_xact_id: null } },
    ...
    },
    {
        ...
        steps: [
            ...
            {
                values: { name: "READ" },
                meta: { snapshot: { xmin: 5, xmax: 5, xip: [], current_xact_id: null } },
                expected: { name: "WRITE" },
...
const MANY_FIELD_CASES = [
    ...
    },
    {
        ...
        steps: [
            ...
            {
                values: [["DELETE", [1, 2, 3]]],
                meta: { snapshot: { xmin: 15, xmax: 15, xip: [], current_xact_id: null } },
                expected: [1, 2, 3],
                description:
...
test("Inverse of relations are properly versioned", async () => {
    ...
    store.insert({
        ...
        __store_version__: { snapshot: { xmin: 1, xmax: 1, xip: [] } },
    });
    ...
    store.insert({
        ...
        __store_version__: { snapshot: { xmin: 3, xmax: 3, xip: [] } },
    });
    ...
    store.insert({
        ...
        __store_version__: { snapshot: { xmin: 2, xmax: 2, xip: [] } },
    });
    expect(store.Message.get(1).thread).toBe(undefined);
    expect(store.Thread.get(1).messages.map((m) => m.id)).toEqual([2]);
    store.insert({
        Message: { id: 1, thread: 1 },
        __store_version__: { snapshot: { xmin: 4, xmax: 4, xip: [] } },
    });
    expect(store.Message.get(1).thread.id).toBe(1);
    expect(
        store.Thread.get(1)
