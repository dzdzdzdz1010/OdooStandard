...
test("Readonly channel member has bottom banner instead of composer", async () => {
    const pyEnv = await startServer();
    ...
    await openDiscuss(channelId);
    await contains(".o-mail-DiscussContent-core span:text('This channel is readonly.')");
    await contains(".o-mail-Composer", { count: 0 });
...
test("Readonly channel admin has composer", async () => {
    const pyEnv = await startServer();
...
test("Readonly channel member cannot respond or create subthread", async () => {
    const pyEnv = await startServer();
    const memberPartnerId = pyEnv["res.partner"].create({ name: "Member User" });
    pyEnv["res.users"].create({
        partner_id: memberPartnerId,
...
test("Readonly channel have reactions for admin", async () => {
    const pyEnv = await startServer();
    const adminPartnerId = pyEnv["res.partner"].create({ name: "Admin User" });
    const channelId = pyEnv["discuss.channel"].create({
        name: "General",
...
test("Cannot call readonly channels", async () => {
    const pyEnv = await startServer();
    const adminPartnerId = pyEnv["res.partner"].create({ name: "Admin User" });
    const channelId = pyEnv["discuss.channel"].create({
        name: "General",
