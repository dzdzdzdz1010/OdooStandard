...
test("can change the thread description of #general", async () => {
    ...
    onRpc("discuss.channel", "channel_change_topic", ({ route }) => expect.step(route));
    ...
    triggerHotkey("Enter");
    await expect.waitForSteps(["/web/dataset/call_kw/discuss.channel/channel_change_topic"]);
...
test("Do not trigger channel description server update when channel has no description and editing to empty description", async () => {
    ...
    onRpc("discuss.channel", "channel_change_topic", ({ method }) => expect.step(method));
