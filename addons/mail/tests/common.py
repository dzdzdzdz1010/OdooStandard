...
class MockEmail(common.BaseCase, MockSmtplibCase):
    ...
    def assertTracking(self, message, data, strict=False):
        ...
        self.assertIn(message.message_type, ('tracking', 'notification'))
        ...
        for item in items:
            ...
            for index, b in enumerate(b_nodes):
                old_value = text_nodes[index].strip()
                new_value = b.text_content().strip()

                field_label = i_nodes[index].text_content().strip()
        ...
        for field_name, value_type, old_value, new_value in data:
            ...
            tracking = match[0]
            field_info = self.env[message.model].fields_get([field_name])[field_name]
            tracking_vals = source_record._prepare_tracking_vals(old_value, new_value, field_name, field_info)
            expected_new_value, expected_old_value = tracking_vals['new_value'], tracking_vals['old_value']

            self.assertEqual(
                tracking["old_value"], str(expected_old_value),
                msg_base + f"expected old={expected_old_value}, got {tracking['old_value']})"
            )

            self.assertEqual(
                tracking["new_value"], str(expected_new_value),
