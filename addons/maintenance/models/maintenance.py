...
class MaintenanceEquipmentCategory(models.Model):
    ...
    def _compute_maintenance_count(self):
        maintenance_data = self.env['maintenance.request']._read_group([('category_id', 'in', self.ids)], ['category_id', 'kanban_state'], ['__count'])
        mapped_data = {(category.id, kanban_state): count for category, kanban_state, count in maintenance_data}
        for category in self:
            category.maintenance_open_count = sum(mapped_data.get((category.id, kanban_state), 0) for kanban_state in ['normal', 'changes_requested', 'approved'])
            category.maintenance_count = category.maintenance_open_count + sum(mapped_data.get((category.id, kanban_state), 0) for kanban_state in ['done', 'cancelled'])
...
class MaintenanceMixin(models.AbstractModel):
    ...
    @api.depends('effective_date', 'maintenance_ids.kanban_state', 'maintenance_ids.close_date', 'maintenance_ids.schedule_date')
    def _compute_maintenance_request(self):
        for record in self:
            maintenance_requests = record.maintenance_ids.filtered(lambda mr: mr.maintenance_type == 'corrective' and mr.kanban_state == 'done')
    ...
    @api.depends('maintenance_ids.kanban_state')
    def _compute_maintenance_count(self):
        for record in self:
            record.maintenance_count = len(record.maintenance_ids)
            record.maintenance_open_count = len(record.maintenance_ids.filtered(lambda mr: mr.kanban_state not in ['done', 'cancelled']))
...
class MaintenanceRequest(models.Model):
    ...
    kanban_state = fields.Selection([
        ...
        ('done', 'Done'),
        ('cancelled', 'Cancelled'),
    ], string='Kanban State', required=True, default='normal', tracking=True, copy=False)
    ...
    def cancel_equipment_request(self):
        self.write({'kanban_state': 'cancelled', 'recurring_maintenance': False})
    ...
    def reset_equipment_request(self):
        ...
        self.write({'kanban_state': 'normal', 'stage_id': first_stage_obj.id})
    ...
    def create(self, vals_list):
        ...
        for request in maintenance_requests:
            ...
            if request.close_date and request.kanban_state != 'done':
                request.close_date = False
            if not request.close_date and request.kanban_state == 'done':
    ...
    def write(self, vals):
        ...
        if vals and 'kanban_state' not in vals and 'stage_id' in vals:
            self.filtered(lambda request: request.kanban_state in ['changes_requested', 'approved']).kanban_state = 'normal'
        ...
        if 'kanban_state' in vals:
            done_requests = self.filtered(lambda request: request.kanban_state == 'done')
            done_requests.close_date = fields.Date.today()
            (self - done_requests).close_date = False
        return res
...
class MaintenanceTeam(models.Model):
    ...
    todo_request_count_changes_requested = fields.Integer(string="Number of Requests in Changes Requested", compute='_compute_todo_requests')
    ...
    @api.depends('request_ids.kanban_state')
    def _compute_todo_requests(self):
        for team in self:
            domain = [('maintenance_team_id', '=', team.id), ('kanban_state', 'not in', ['done', 'cancelled'])]
            team.todo_request_ids = self.env['maintenance.request'].search(domain)
            data = self.env['maintenance.request']._read_group(domain, ['schedule_date:year', 'priority', 'kanban_state'], ['__count'])
            ...
            team.todo_request_count_changes_requested = sum(count for (_, _, kanban_state, count) in data if kanban_state == 'changes_requested')
