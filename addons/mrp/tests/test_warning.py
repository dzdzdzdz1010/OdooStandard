...
class TestConsumptionWarning(common.TransactionCase):
    ...
    # Scenarios to test:
    # 0. No changes:
    #   The wizard must not display anything.
    # 1. A component move is missing:
    #   It must be reported as missing (consumed quantity: 0).
    # 2. A component is too high or too low in quantity:
    #   It must be reported as mismatching.
    # 3. A component that wasn't in the BoM is added:
    #   It must be reported as foreign (expected quantity: 0).
    # 4. A component is removed but readded verbatim: It must not be reported (despite the fact that the move
    #   has no associated BoM line) if the move's product and UoM (if enabled) match the orphaned BoM line.
    # 5. A component is removed and readded in a different way (different UoM, split into multiple moves etc):
    #   It must be reported as missing (consumed quantity: 0) and the readded moves will be considered foreign
    #   (expected quantity: 0). We don't add up moves of the same product even if they have the same UoM, and
    #   treat them all distinctly.
    # 6. There is no BoM:
    #   The expected quantity of the move must be used as reference to compare against instead.
    #
    # For every case the warning was triggered, test the 'Set Quantities & Validate' option.
    ...
    def test_consumption_warning_with_uom_and_kits(self):
        ...
        for ingredient in ingredients:
            if ingredient.product_id == self.msg:
                ...
                # -> 3600 packs of ramen in 1 m³ soup
                # -> 7200 g MSG in 1 m³ soup
