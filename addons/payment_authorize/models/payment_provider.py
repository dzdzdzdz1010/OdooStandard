...
class PaymentProvider(models.Model):
    ...
    def action_create_webhook(self):
        ...
        if self.state == 'disabled':
            raise UserError(self.env._("Cannot create webhook while the provider is disabled."))
        ...
        webhook_data = {
            'name': 'Odoo webhook',
            'url': webhook_url,
            'eventTypes': const.WEBHOOK_HANDLED_EVENTS,
            'status': 'active',
        }

        response = self._send_api_request(
            'POST', '/rest/v1/webhooks', json=webhook_data
        )

        if webhook_id := response.get('webhookId'):
            self.authorize_webhook_id = webhook_id

        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': self.env._("Webhook Created"),
    ...
    def _authorize_get_api_url(self):
        """Return the base API URL for Authorize.Net based on the provider state.
    ...
    def _build_request_url(self, endpoint, **kwargs):
        ...
        return f'{self._authorize_get_api_url()}{endpoint}'

    def _build_request_headers(self, method, endpoint, payload, **kwargs):
        """Override of `payment` to build the headers for Authorize.Net REST API."""
        if self.code != 'authorize':
