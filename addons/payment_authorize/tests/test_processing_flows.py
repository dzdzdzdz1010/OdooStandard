...
class TestProcessingFlows(AuthorizeCommon, PaymentHttpCommon):
    ...
    def test_webhook_notification_triggers_processing(self):
        ...
        payment data."""
        self._create_transaction('direct', provider_reference=self.trans_id)
    ...
    @mute_logger('odoo.addons.payment_authorize.controllers.main')
    def test_webhook_notification_accepts_valid_signature(self):
        ...
        AuthorizeController._verify_signature(signature_header, body, tx)
    ...
    def test_webhook_notification_rejects_invalid_signature(self):
        ...
        tx = self._create_transaction('direct', provider_reference=self.trans_id)
        body = json.dumps(self.webhook_authcapture_data).encode('utf-8')
        signature_header = 'sha512=INVALIDSIGNATURE'

        self.assertRaises(
            Forbidden, AuthorizeController._verify_signature, signature_header, body, tx
        )
