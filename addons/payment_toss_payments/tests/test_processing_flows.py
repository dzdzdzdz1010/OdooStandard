...
class TestProcessingFlows(TossPaymentsCommon, PaymentHttpCommon):
    ...
    def test_webhook_triggers_apply_updates(self):
        """ Test receiving a valid webhook notification triggers the processing of the payment data.
        """
    ...
    def test_accept_notification_with_valid_signature(self):
        ...
        tx = self._create_transaction('direct')
        tx.write({
            'toss_payments_payment_secret': 'test-secret',
        })
    ...
    def test_success_flow_api_validation_error_triggers_set_error(self):
        """ If sending the confirm API request raises ValidationError, the tx must be set to error.
        """
        tx = self._create_transaction('direct')
        redirect_success_params = {
            'orderId': tx.reference,
