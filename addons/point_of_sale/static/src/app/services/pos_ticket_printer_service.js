...
import { RenderContainer } from "../components/render_container/render_container";

export const posTicketPrinterService = {
    dependencies: ["dialog", "pos_data", "notification"],
    start(env, dependencies) {
        return new PosTicketPrinterService(env, dependencies);
    },
};
...
export class PosTicketPrinterService {
    ...
    setup(env, { dialog, pos_data, notification }) {
        ...
        // This container is only available when all PoS services have finished to load.
        registry.category("main_components").add("RenderContainer", {
            Component: RenderContainer,
            props: { component: this.componentStates },
        });
    ...
    getOrderReceiptData(order, basic = false) {
        const generator = this.getGenerator(this.data.models, order, basic);
    ...
    async openCashbox() {
        if (!this.config.default_receipt_printer_id?._action?.openCashbox) {
            return false;
        }

        return await this.config.default_receipt_printer_id._action.openCashbox();
    }
    ...
    async print({ printer, iframe, image = null }) {
        ...
        let status;
        try {
            status = await printer._action.print(finalImage);
        } catch {
            status = {
                successful: false,
     ...
     */
    async printSaleDetailsReceipt() {
        const generator = this.getGenerator(this.data.models);
        ...
        return await this.printWithFallback({ iframe });
    }

    async printTipReceipt({ order, name }) {
        const generator = this.getGenerator(this.data.models, order);
        const data = generator.generateTipData(name);
        const iframe = await this.generateIframe("point_of_sale.pos_tip_receipt", data);
        return await this.printWithFallback({ iframe });
    }

    async printCashMoveReceipt({ reason, translatedType, order, formattedAmount }) {
    ...
    async printOrderReceipt({ order, basic = false, printBillActionTriggered = false } = {}) {
        ...
        const result = await this.printWithFallback({ iframe });
    ...
    async printOrderChanges({ order, opts = {}, printers = this.config.preparation_printer_ids }) {
        ...
        if (unsuccessfulPrints.length) {
            const message = {
                title: _t("Some printers failed to print"),
    ...
    async initPrinters() {
        ...
        for (const printer of printers) {
            const action = await this.createPrinterInstance(printer);
            printer._action = action;
        }
    }
    ...
    async getHtmlFromComponent(ComponentClass, data) {
        return new Promise((resolve) => {
            Object.assign(this.componentStates, {
                data,
                component: ComponentClass,
                onRendered: (el) => resolve(el?.innerHTML),
            });
        });
    }
