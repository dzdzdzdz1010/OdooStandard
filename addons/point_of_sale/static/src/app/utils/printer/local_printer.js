...
export class LocalPrinter extends BasePrinter {
    ...
    async _initializePrinter() {
        try {
            const printers = await window.printer.getDefaultPrinter();

            if (printers.status && printers.printer) {
                const printer_info = printers.printer[0];

                this.vendor_id = printer_info.vendor_id;
                this.product_id = printer_info.product_id;
                this.product = printer_info.product || "Unknown Printer";
                return true;
            } else {
                return false;
            }
    ...
    async openCashbox() {
        await this.sendPrintingJob();
    }

    async sendPrintingJob(printData = "") {
        try {
            const available = await this._initializePrinter();
            if (!available) {
                return this._getErrorResult(ERROR_CODES.PRINTER_NOT_FOUND);
            }

            const response = await window.printer.printReceipt({
                name: this.product,
