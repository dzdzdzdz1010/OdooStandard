...
test("getPaymentActionState", async () => {
    ...
    const stateReversing = comp.getPaymentActionState(paymentline);
    expect(normalizeActionState(stateReversing)).toEqual({
        id: "reversing",
        icon: "fa fa-circle-o-notch fa-spin",
        title: "Reversal request sent to terminal",
        actions: [],
    });

    // Done - can be reversed - refund
    paymentline.payment_status = "done";
    paymentline.canBeReversed = true;
    comp.props.isRefundOrder = true;
    const stateDoneRefundCanBeReversed = comp.getPaymentActionState(paymentline);
    expect(normalizeActionState(stateDoneRefundCanBeReversed)).toEqual({
        id: "refunded",
    ...
    // Done - cannot be reversed - no refund
    paymentline.canBeReversed = false;
    comp.props.isRefundOrder = false;
    const stateDoneNoRefundCannotBeReversed = comp.getPaymentActionState(paymentline);
    expect(normalizeActionState(stateDoneNoRefundCannotBeReversed)).toEqual({
        id: "paid",
    ...
    const stateReversed = comp.getPaymentActionState(paymentline);
    expect(normalizeActionState(stateReversed)).toEqual({
        id: "reversed",
