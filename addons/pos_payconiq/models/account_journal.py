...
class AccountJournal(models.Model):
    ...
    def _check_currency(self):
        ...
        for record in self:
            ...
            if any(pm.payment_provider == "payconiq" for pm in record.pos_payment_method_ids):
                raise ValidationError(
                    self.env._(
                        "The journal '%(journal)s' is linked to a Payconiq payment method, which only supports the following currencies: %(currencies)s.\n"
