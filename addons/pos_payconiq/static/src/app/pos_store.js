...
patch(PosStore.prototype, {
    async setup() {
        ...
        this.handlePayconiqNotification = this.handlePayconiqNotification.bind(this);
        this.data.connectWebSocket(
            "PAYCONIQ_PAYMENTS_NOTIFICATION",
            this.handlePayconiqNotification
        );
    },

    predicateGetPaymentToForceCancel(payment) {
        return (
            super.predicateGetPaymentToForceCancel(...arguments) ||
            (payment.payment_provider === "payconiq" && payment.payment_status === "waiting")
        );
    },

    async handlePayconiqNotification({ payment_id, payconiq_status }) {
        ...
        // --- FAILED ---
            ...
            ["AUTHORIZATION_FAILED", "FAILED", "EXPIRED", "CANCELLED"].includes(payconiq_status)
        ) {
            paymentLine.updateCustomerDisplayQrCode(null);
            const message = this.getPayconiqErrorMessage(payconiq_status, order);
    ...
    getPayconiqErrorMessage(status, order) {
    ...
    canSendPaymentRequest({ paymentMethod, paymentline }) {
        ...
        if (
            paymentMethod.payment_provider === "payconiq" &&
            paymentMethod.payconiq_usage === "display"
        ) {
