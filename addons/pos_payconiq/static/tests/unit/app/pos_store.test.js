...
describe("getPayconiqErrorMessage", () => {
    test("EXPIRED", async () => {
        ...
        store.setOrder(null);
        const actualUnselectedOrder = store.getPayconiqErrorMessage("EXPIRED", order);
        expect(actualUnselectedOrder).toBe("A payment for order test_order has expired.");

        store.setOrder(order);
        const actualCurrentOrder = store.getPayconiqErrorMessage("EXPIRED", order);
    ...
    test("CANCELLED", async () => {
        ...
        store.setOrder(null);
        const actualUnselectedOrder = store.getPayconiqErrorMessage("CANCELLED", order);
        expect(actualUnselectedOrder).toBe("A payment for order test_order was cancelled.");

        store.setOrder(order);
        const actualCurrentOrder = store.getPayconiqErrorMessage("CANCELLED", order);
    ...
    test("OTHER", async () => {
        ...
        store.setOrder(null);
        const actualUnselectedOrder = store.getPayconiqErrorMessage("AUTHORIZATION_FAILED", order);
        expect(actualUnselectedOrder).toBe("A payment for order test_order has failed.");

        store.setOrder(order);
        const actualCurrentOrder = store.getPayconiqErrorMessage("AUTHORIZATION_FAILED", order);
        expect(actualCurrentOrder).toBe("Payment failed");
    });
});

describe("handlePayconiqNotification", () => {
    test("SUCCEEDED", async () => {
        ...
        await store.handlePayconiqNotification({
            payment_id: paymentline.id,
            payconiq_status: "SUCCEEDED",
        });
