...
patch(PosStore.prototype, {
    ...
    async restoreOrdersToOriginalTable(order, unmergeTable) {
        ...
        if (beforeMergeDetails?.length) {
            const prepLineUuids = beforeMergeDetails.map((bm) => bm.prepLineUuids).flat();
            ...
            for (const detail of beforeMergeDetails) {
                ...
                if (prepLines?.length) {
                    detail.quantity = prepLines.reduce(
                        (sum, l) => sum + l.quantity - l.cancelled,
                        0
                    );
                } else {
                    detail.quantity = Math.min(detail.quantity, line.qty);
                }
    ...
    computeTableCount(data) {
        ...
        for (const table of tables) {
            ...
            table.uiState.changeCount = order.preparationChanges.quantity;
        }
    },
    get selectedTable() {
    ...
    async _askForPreparation() {
        ...
        if (
            this.config.module_pos_restaurant &&
            this.getOrder().preparationChanges.categoryCount.length &&
            !order.isRefund
        ) {
