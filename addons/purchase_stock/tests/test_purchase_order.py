...
class TestPurchaseOrder(ValuationReconciliationTestCommon):
    ...
    def test_log_activity_in_po_with_receipt_without_backorders(self):
        ...
        """
        expected_note = """<div>
            Received quantity is less than the ordered quantity for transfer
            ...
            <div class="mt16">
                <ul>
                    <li>
                        Only 50.0 Units
                        of product_a
                        received in contrast to the ordered 200.0 Units
                    </li>
                </ul><ul>
                    <li>
                        Only 5.0 Dozens
                        of product_b
                        received in contrast to the ordered 20.0 Dozens
                    </li>
                </ul>
            </div>
        </div>"""
        ...
        receipt.move_line_ids.search([('product_id', '=', self.product_a.id)]).write({'quantity': 50})
        receipt.move_line_ids.search([('product_id', '=', self.product_b.id)]).write({'quantity': 5})
        receipt.move_line_ids.search([('product_id', '=', product_c.id)]).write({'quantity': 2})
        ...
        second_step.move_line_ids.search([('product_id', '=', self.product_a.id)]).write({'quantity': 50})
        second_step.move_line_ids.search([('product_id', '=', self.product_b.id)]).write({'quantity': 5})
        second_step.move_line_ids.search([('product_id', '=', product_c.id)]).write({'quantity': 2})
