...
class AccountMove(models.Model):
    ...
    def _link_analytic_lines_to_invoice(self):
        ...
        related analytic lines with the invoice by setting their `reinvoice_id` field.
        ...
        for invoice in invoices:
            invoice._get_analytic_lines_to_link().write({'reinvoice_id': invoice.id})
    ...
    def _get_analytic_lines_to_link(self):
        ...
        so_lines = self.line_ids.sale_line_ids.filtered(self._filter_sale_lines_to_reinvoice)
    ...
    def _filter_sale_lines_to_reinvoice(self, so_line):
        """Filter hook to get sale lines that should be re-invoiced."""
        return not so_line.is_expense and so_line.product_id.invoice_policy == 'delivery'
