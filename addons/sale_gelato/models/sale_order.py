...
class SaleOrder(models.Model):
    ...
    def _onchange_validate_partner_shipping_address(self):
        """Warn if the address doesn't comply with Gelato requirements. Since partner_shipping_id
        is not always present in the view, partner_id is also a trigger to ensure the address
        validation.
        """
        for order in self.filtered(
            ...
            if message := shipping_address._gelato_validate_delivery_address():
                warning = {'title': _("The address is incorrect."), 'message': message}
