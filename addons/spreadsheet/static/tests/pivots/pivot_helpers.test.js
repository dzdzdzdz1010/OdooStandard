...
test("Basic formula extractor", async function () {
    ...
    const compiledFormula = CompiledFormula.CompileFormula(formula, "no_sheet", model.getters);
    let functionName;
    let args;
...
test("Number of formulas", async function () {
    const model = new Model();
    const formula = CompiledFormula.CompileFormula(`=PIVOT.VALUE("1", "test") + PIVOT.VALUE("2", "hello") + ODOO.LIST("1", 1, "bla")`, "no_sheet",model.getters);
    expect(getNumberOfPivotFunctions(formula,model.getters)).toBe(2);
    expect(hasListFormula(formula, model.getters)).toBe(true);
    const formula2 = CompiledFormula.CompileFormula("=1+1", "no_sheet",model.getters);
    expect(getNumberOfPivotFunctions(formula2, model.getters)).toBe(0);
    expect(hasListFormula(formula2, model.getters)).toBe(false);
    const formula3 = CompiledFormula.CompileFormula("=bla", "no_sheet",model.getters);
...
test("getFirstPivotFunction does not crash when given crap", async function () {
    const model = new Model();
    expect(getFirstListFunction(CompiledFormula.CompileFormula("=SUM(A1)","no_sheet", model.getters))).toBe(undefined);
    expect(getFirstPivotFunction(CompiledFormula.CompileFormula("=SUM(A1)","no_sheet", model.getters))).toBe(undefined);
    expect(getFirstListFunction(CompiledFormula.CompileFormula("=1+1","no_sheet", model.getters))).toBe(undefined);
    expect(getFirstPivotFunction(CompiledFormula.CompileFormula("=1+1","no_sheet", model.getters))).toBe(undefined);
    expect(getFirstListFunction(CompiledFormula.CompileFormula("=bla","no_sheet", model.getters))).toBe(undefined);
    expect(getFirstPivotFunction(CompiledFormula.CompileFormula("=bla","no_sheet", model.getters))).toBe(undefined);
    expect(getFirstListFunction(CompiledFormula.CompileFormula("bla","no_sheet", model.getters))).toBe(undefined);
    expect(getFirstPivotFunction(CompiledFormula.CompileFormula("bla","no_sheet", model.getters))).toBe(undefined);
});
