...
class StockPicking(models.Model):
    ...
    def action_confirm(self):
        ...
        if not self.env.context.get('skip_zero_demand_check'):
            ...
            if zero_demand_moves:
                return self._action_generate_zero_demand_wizard(zero_demand_moves)
    ...
    def action_return_all(self):
        ...
        if self.return_id:
            for return_move in self.move_ids.filtered(lambda m: m.state not in ('done', 'cancel')):
                ...
                quantity = return_move.product_uom.round(quantity)
    ...
    def _prepare_return_move_default_values(self, move_id):
        ...
        vals = {
            ...
            'product_uom': move_id.product_uom.id or move_id.product_id.uom_id.id,
            'picking_id': self.id,
            'state': 'draft',
    ...
    def _action_generate_zero_demand_wizard(self, zero_demand_moves):
        ...
        return {
            ...
            'context': dict(
                ...
                default_product_ids=[m.product_id.id for m in zero_demand_moves],
            ),
        }
