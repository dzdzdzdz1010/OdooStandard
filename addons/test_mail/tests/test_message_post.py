...
class TestMessagePost(TestMessagePostCommon, CronMixinCase):
    ...
    @users('employee')
    def test_post_duplicate_followers(self):
        ...
        for (
            ...
            with self.subTest(case_name=case_name):
                self.test_record.message_partner_ids = False
                self.test_record.message_subscribe((internal_followers + external_followers).ids)
                ...
                if notified_internal_partners := (notified_partners & internal_partners) & mail_notified_partners:
                    self.assertMailMail(notified_internal_partners, 'sent', author=self.env.ref('base.partner_root'), mail_message=message)
                if notified_customer_partners := (notified_partners - internal_partners) & mail_notified_partners:
                    self.assertMailMail(notified_customer_partners, 'sent', author=self.env.ref('base.partner_root'), mail_message=message)
                ...
                if email_to_all := [addr for addr, count in notified_emails.items() if count]:
                    self.assertMailMail(
                        ...
                        author=self.env.ref('base.partner_root'), mail_message=message,
                    )
