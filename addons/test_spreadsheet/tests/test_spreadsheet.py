...
class ValidateSpreadsheetMixinData(TransactionCase):
    def test_onchange_json_data(self):
        ...
        spreadsheet_form.spreadsheet_binary_data = BinaryValue.from_bytes(json.dumps({'key': 'value'}).encode('utf-8'))
        with self.assertRaises(ValidationError, msg='Invalid JSON Data'):
            spreadsheet_form.spreadsheet_binary_data = BinaryValue.from_bytes(b'invalid json')
