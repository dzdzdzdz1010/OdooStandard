...
/**
 * `initialValue` needs to be cleared when it is checked.
 *
 * @param {string} nextValue
 */
function hasValueChanged(nextValue) {
    const hasChanged = runTime.initialValue !== nextValue;
...
async function registerForChange(target, confirmAction) {
    ...
    if (getTag(target) === "input") {
        runTime.changeTargetListeners.push(
            on(parentView, "keydown", function onWindowKeyDown(ev) {
...
async function triggerChange() {
    ...
    if (hasValueChanged(target.value)) {
...
async function _keyDown(targetResolver, eventInit, options) {
     ...
     * @param {string} type
     */
    function insertValue(target, toInsert, type) {
        const { selectionStart, selectionEnd, value } = target;
        inputData = toInsert;
        inputType = type;
    ...
    let nextValue = inputTarget.value;
