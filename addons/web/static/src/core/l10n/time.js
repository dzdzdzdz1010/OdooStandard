...
export const durationUnitsRegex = memoize(() => {
    const durationUnits = new Intl.DurationFormat(localization.locale, { style: "narrow" })
        .formatToParts({
            hours: 1,
            minutes: 1,
            seconds: 1,
        ...
        .map((d) => d.value[0] + (d.value.slice(1) ? `[${d.value.slice(1)}]*` : ""));

    const regexTimes = {
        hours: new RegExp("(\\d+)" + durationUnits[0]),
        minutes: new RegExp("(\\d+)" + durationUnits[1]),
        seconds: new RegExp("(\\d+)" + durationUnits[2]),
    };
