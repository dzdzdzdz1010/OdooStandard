...
function translatePoint(point, vector) {
    return IDENTITY_MATRIX.translate(vector.x, vector.y).transformPoint(point);
}
function scaleVector(vector, ...args) {
    return IDENTITY_MATRIX.scale(...args).transformPoint(vector);
}
...
function pointerInsideElementOffset(pointer, elementRect) {
    const pointerView = pointer.view.deref?.() ?? (pointer.view || window);
    ...
    return {
        [hookName](params) {
            ...
            const updateElementPosition = () => {
                ...
                const pointerView = ctx.pointer.view?.deref() ?? (pointer.view || window);
            ...
            const updatePointerPosition = (ev) => {
                ...
                ctx.pointer.view = new WeakRef(ev.view || window);
            };
            ...
            const ctx = {
                ...
                followCursor: true,
                cursor: null,
                pointer: { x: 0, y: 0, view: new WeakRef(window) },
