...
test(`[Offline] save a form view offline (click save icon)`, async () => {
    ...
    expect(queryAllTexts`.o-dropdown--menu`).toEqual(["Partner\nfirst record to be edited"]);
...
test(`[Offline] save a form view offline (autosave when leaving)`, async () => {
    ...
    expect(".o_data_cell:first").toHaveText("yop"); // TODO: It should be nice to reload the current view after sync ?? For me it should be "new foo"
    expect.verifySteps(["web_save"]);
});
