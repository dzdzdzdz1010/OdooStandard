...
export class ToggleBodyBgImageAction extends BuilderAction {
    ...
    async applyStateWithLoader(state) {
        this.services.ui.block({ delay: 2500 });
        try {
            await this.setBodyBgState(state);
        } finally {
            this.services.ui.unblock();
        }
    }
    async setBodyBgState(state) {
        // Store the current body bg selection (image + type).
        const variables = {
            "body-image-type": `'${state.type}'`,
            "body-image": state.image ? `'${state.image}'` : "",
        };
        if (!state.image) {
    ...
    async commitState(oldState, newState) {
        await this.applyStateWithLoader(newState);
        this.dependencies.history.addCustomMutation({
            apply: () => this.applyStateWithLoader(newState),
            revert: () => this.applyStateWithLoader(oldState),
        });
    }
    async apply({ editingElement: el } = {}) {
        const getAction = this.dependencies.builderActions.getAction;
        const { type: currentType, image: currentImage } = this.getCurrentState();
        const oldState = { type: currentType, image: currentImage };

        const imageEl = await getAction("replaceBgImage").load({ el });
        if (!imageEl) {
            return;
        }
        const newState = { type: currentType, image: imageEl.src };
        await this.commitState(oldState, newState);
    }
    async clean() {
        const { type: currentType, image: currentImage } = this.getCurrentState();
        const oldState = { type: currentType, image: currentImage };
        const newState = { type: "image", image: "" };
        await this.commitState(oldState, newState);
    }
}
...
export class BodyBgPositionOverlayAction extends BuilderAction {
        ...
        return new Promise((resolve) => {
            const removeOverlay = this.services.overlay.add(ImagePositionOverlay, {
                ...
                onDrag: (percentPosition) => {
                    // Live preview via inline style; cleared on apply().
    ...
    async apply({ editingElement, loadResult: bgPosition }) {
