...
export class DynamicSnippetCarousel extends DynamicSnippet {
    ...
    setup() {
        ...
        this.holderEl = this.el.querySelector(".s_dynamic_content_holder");
        this.contentEl = this.el.querySelector(".s_dynamic_snippet_content");
    ...
    setLoading(isLoading) {
        if (isLoading) {
            ...
            this.holderEl.classList.remove("d-none");
        } else {
            this.holderEl.classList.add("d-none");
     ...
     */
    getActiveSlideIndex() {
        const slideEl = this.carouselEl.querySelectorAll(".carousel-item");
        return [...slideEl].findIndex((s) => s.classList.contains("active"));
    }
    ...
    /**
     * Updates the visibility of the "Previous" navigation button.
     *
    ...
    togglePrevButton() {
        ...
        if (!this.carouselEl || !prev) {
            return;
        }
        if (this.getScrollMode() === "single") {
            prev.classList.toggle("d-none", this.hasMore && this.index == 0);
        } else {
            prev.classList.toggle("d-none", this.hasMore && this.getActiveSlideIndex() === 0);
        }
    }
    ...
    appendContent() {
        ...
        const newSlides = [...newInnerEl.children];
        newSlides[0].classList.remove("active");
        if (this.getScrollMode() === "single") {
            const referenceNode = oldInnerEl.children[this.getItemsPerSlide()];
            newSlides.forEach((el) => oldInnerEl.insertBefore(el, referenceNode));
        } else {
            oldInnerEl.append(...newSlides);
        }
