...
options.registry.GalleryLayout = options.registry.CarouselHandler.extend({
     ...
     */
    _grid() {
        const mediaHolderEls = this._getMediaHolderEls();
...
options.registry.GalleryImageList = options.registry.GalleryLayout.extend({
    ...
    start() {
        ...
        this.$target.on("image_changed.gallery", "img, .media_iframe_video", async (ev) => {
            // Handles the update of the thumbnail when an image or a video is
            // changed.
            const $media = $(ev.currentTarget);
            if (ev.indexAttr) {
                $media.attr("data-index", ev.indexAttr);
            }
            ...
            const src = $media.is("img") ? $media.attr("src") : await this._getVideoThumbnailUrl($media.data("oeExpression"));
            $media.hasClass("media_iframe_video") ? this._addVideoPlayIcon($indicator[0]) : $indicator.empty().removeClass("o_not_editable");
            $indicator.css("background-image", `url(${src})`);
        });
