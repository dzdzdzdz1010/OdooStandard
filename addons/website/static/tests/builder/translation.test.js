...
test("translation span wrapper for <option>", async () => {
    ...
    expect(":iframe .o_translation_select > div > span[data-oe-translation-source-sha]").toHaveCount(1, {
        message: "Only the first option has the translation span wrapper",
    });
    expect(":iframe .o_translation_select > div > span[data-oe-translation-source-sha]").toHaveText("Première option");
});
