...
import { click, queryAll, animationFrame } from "@odoo/hoot-dom";
...
test("dynamic snippet loads items and displays them through template", async () => {
    ...
    expect(itemEls[6]).toHaveAttribute("data-test-param", "test7");
    ...
    expect(itemEls[6]).toHaveAttribute("data-started", "*test7*");
    expect(itemEls[7]).toHaveAttribute("data-started", "*test8*");
    await animationFrame();
    await click(".s_dynamic_snippet_load_more");
