...
class WebsiteCrmPartnerAssign(WebsitePartnership, GoogleMap):
    ...
    def _get_partners_values(self, country=None, grade=None, page=0, references_per_page=20, **post):
        ...
        if request.website.is_view_active("website_crm_partner_assign.index_filter_by_industry") and current_industry:
    ...
    def partners_detail(self, partner_id, **post):
        if country_id := post.get('country_id'):
            ...
            post['additional_render_values'] = post.get('additional_render_values', {}) | {'current_country': current_country}
        if not post.get('template'):
            post.update({'template': "website_crm_partner_assign.partner"})
