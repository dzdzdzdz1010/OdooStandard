...
class Website(models.Model):
    ...
    def _get_company_markup_data(self):
        ...
        return {
            ...
            'name': self.name,
            'url': base_url,
            'logo': f"{base_url}/logo.png?company={self.company_id}",
            'sameAs': [social for social in socials if social],
        }

    def get_company_markup_json(self):
        """Dump the data to JSON for the template."""
        company_markup_data = self._get_company_markup_data()
        company_markup_data['@context'] = 'https://schema.org'
        return json_scriptsafe.dumps(company_markup_data, indent=2)
