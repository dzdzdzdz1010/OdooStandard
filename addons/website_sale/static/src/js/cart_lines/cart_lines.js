...
export class CartLines extends Component {
	...
	setup() {
        ...
        useBus(this.cartService.bus, 'cart_update', ({ detail: { cartLinesData } }) => {
            // ewww wtf is this?
            if (cartLinesData) {
                if (cartLinesData.cart_lines.length) {
                    this.updateLines(cartLinesData);
                } else {
                    redirect('/shop/cart');
                }
            }
    ...
    async updateLines(cartLines = false) {
        const data = cartLines || await rpc('/shop/cart/lines');
    ...
    // TEMPORARY HACK TO MAKE RUNBOT HAPPY, WISHLIST WILL BE MERGED IN WEBSITE_SALE ANYWAYS
    addToWishlist(lineId, productId) {
        return true;
    }
