...
export class CartTotal extends Component {
    ...
    setup() {
        ...
        useBus(this.cartService.bus, 'cart_update', ({ detail: { cartTotalsData } }) => {
            this.updateTotals(cartTotalsData);
        });
    }

    async updateTotals(cartTotalsData = false) {
        const data = cartTotalsData || await rpc('/shop/cart/totals');
