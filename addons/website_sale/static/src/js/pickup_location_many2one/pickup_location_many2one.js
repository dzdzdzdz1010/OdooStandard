...
export class PickupLocationMany2OneField extends Many2OneField {
    ...
    async onWillStart() {
        const orderId = this.props.record.resModel === "sale.order" ? this.props.record.resId : this.props.record.data.order_id.id;
        this.state.isLoading = true;
        if (this.props.record.data.carrier_id) {
            const rateData = await this.orm.call("delivery.carrier", "rate_shipment_for_order", [this.props.record.data.carrier_id.id, orderId]);
