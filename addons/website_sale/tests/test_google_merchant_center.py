...
class TestWebsiteSaleGMC(WebsiteSaleGMCCommon, HttpCase):
    ...
    def test_gmc_items_link_redirects_to_correct_product_case_specific_pricelist(self):
        ...
        for product in self.red_sofa + self.blue_sofa:
            ...
            attribute_values = product.product_template_attribute_value_ids.product_attribute_value_id
            attribute_value_params = [
                f'{slug(attribute_value.attribute_id)}={slug(
                    attribute_value.with_context(show_attribute=False)
