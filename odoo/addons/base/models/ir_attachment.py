...
class IrAttachment(models.Model):
    ...
    def _file_read(self, fname):
        ...
        full_path = self._full_path(fname)
        try:
            f = open(full_path, 'rb')  # noqa: SIM115
            return BinaryValue.from_file(f.name, reader=f)
        except OSError:
            _logger.info("_file_read reading %s", full_path, exc_info=True)
            return BinaryValue.from_bytes(b'')
    ...
    def _compute_datas(self):
        for attach in self:
            value = attach.raw
            if value:
                value = BinaryValue.from_bytes(base64.b64encode(value))
            attach.datas = value
    ...
    def _inverse_raw(self):
        self._set_attachment_data(lambda a: a.raw or BinaryValue.from_bytes(b''))

    def _inverse_datas(self):
        warnings.warn("Since 20.0, assign directly to ir.attachment.raw instead of datas", DeprecationWarning)
        self._set_attachment_data(lambda attach: BinaryValue.from_bytes(base64.b64decode(attach.datas or b'')))
        ...
        if str2bool(max_resolution, True):
            try:
                ...
                if w > nw or h > nh:
                    ...
                    values['raw'] = BinaryValue.from_bytes(output)
    ...
    def _check_contents(self, values, *, accept_datas=False):
        ...
        raw = self._fields['raw'].convert_to_cache(raw, self) or BinaryValue.from_bytes(b'')
    ...
    def _to_binary_value(self) -> BinaryValueAttachment:
        """ BinaryValue from the attachment. """
        return BinaryValueAttachment(self.ensure_one())
