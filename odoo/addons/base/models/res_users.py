...
class ResUsersApikeys(models.Model):
    ...
    def generate(self, key, scope, name, expiration_date):
        ...
        if not str2bool(ICP.get_param('base.enable_programmatic_api_keys', False)):
        ...
        nb_keys = self.search_count([('user_id', '=', self.env.uid), ('expiration_date', '>=', self.env.cr.now())])
        try:
        ...
        if nb_keys >= nb_keys_limit:
            raise UserError(_('Limit of %s API keys is reached', nb_keys_limit))
        ...
        logout_env = api.Environment(self.env.cr, None, {})
        uid = logout_env['res.users.apikeys']._check_credentials(scope=scope or 'rpc', key=key)
        if not uid or uid != self.env.uid:
            raise UserError(_("The provided API key is invalid or does not belong to the current user"))
    ...
    def revoke(self, key):
        ...
        """
        if not self.env['ir.config_parameter'].sudo().get_param('base.enable_programmatic_api_keys'):
            raise UserError(_("Programmatic API keys are not enabled"))
