...
class TestBinaryValue(TransactionCase):
    ...
    def test_binary_open(self):
        ...
        val = BinaryFile('base/tests/files/file.csv')
        with self.subTest("Can open multiple times (from_file)"):
            for i in range(1, 3):
                with val.open() as f:
                    self.assertEqual(len(f.read()), 18, f"Read {i}")
        with self.subTest("Check instance of previously opened file"):
