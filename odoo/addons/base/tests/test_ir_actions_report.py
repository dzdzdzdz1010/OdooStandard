...
from odoo.tests.common import tagged, TransactionCase, HttpCase, save_test_file
...
class TestReportsRenderingCommon(HttpCase):
    ...
    def save_pdf(self):
        ...
        save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@tagged('post_install', '-at_install', 'pdf_rendering')
...
@tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
