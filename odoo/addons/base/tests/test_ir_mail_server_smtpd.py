...
from odoo.tools import BinaryValue, config, file_path, mute_logger
...
class TestIrMailServerSMTPD(TransactionCaseWithUserDemo):
    ...
    def test_authentication_certificate_matrix(self):
        ...
        self_signed_key = BinaryValue.from_bytes(self.ssl_self_signed.key.read_bytes())
        self_signed_cert = BinaryValue.from_bytes(self.ssl_self_signed.cert.read_bytes())
        client_key = BinaryValue.from_bytes(self.ssl_client.key.read_bytes())
        client_cert = BinaryValue.from_bytes(self.ssl_client.cert.read_bytes())
        matrix = [
    ...
    def test_man_in_the_middle_matrix(self):
        ...
        mail_server = self.env['ir.mail_server'].create({
            ...
            'smtp_pass': PASSWORD,
            'smtp_ssl_certificate': BinaryValue.from_bytes(self.ssl_client.cert.read_bytes()),
            'smtp_ssl_private_key': BinaryValue.from_bytes(self.ssl_client.key.read_bytes()),
        })
