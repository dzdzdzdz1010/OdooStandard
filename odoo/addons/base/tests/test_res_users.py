...
class TestApiKeys(UsersCommonCase):
    ...
    def test_generate_apikey_is_disabled_by_default(self):
        self.env['ir.config_parameter'].set_param('base.enable_programmatic_api_keys', None)

        # Attempting to create an key raises an error
        with self.assertRaisesRegex(UserError, 'Programmatic API keys are not enabled'):
            self.env['res.users.apikeys'].with_user(self.user_internal).generate(self.unscoped_key, None, 'Another key without a scope', self.tomorrow)
    ...
    def test_generate_apikey_raises_when_creating_unscoped_key_from_scoped_key(self):
        ...
        with self.assertRaisesRegex(UserError, 'The provided API key is invalid or does not belong to the current user'):
            self.env['res.users.apikeys'].with_user(self.user_internal).generate(self.scoped_key, None, 'Another key without a scope', self.tomorrow)
    ...
    def test_generate_apikey_raises_when_creating_key_from_differently_scoped_key(self):
        ...
        with self.assertRaisesRegex(UserError, 'The provided API key is invalid or does not belong to the current user'):
            self.env['res.users.apikeys'].with_user(self.user_internal).generate(self.scoped_key, 'other', 'Another key without a scope', self.tomorrow)
    ...
    def test_generate_apikey_accepts_creating_key_from_identically_scoped_key(self):
        ...
        self.env['res.users.apikeys'].with_user(self.user_internal).generate(self.scoped_key, 'scope', 'Another key without a scope', self.tomorrow)
    ...
    def test_generate_apikey_accepts_creating_scoped_key_from_unscoped_key(self):
        ...
        self.env['res.users.apikeys'].with_user(self.user_internal).generate(self.unscoped_key, 'scope', 'Another key without a scope', self.tomorrow)
    ...
    def test_generate_apikey_checks_ownership(self):
        ...
        with self.assertRaisesRegex(UserError, 'The provided API key is invalid or does not belong to the current user'):
            self.env['res.users.apikeys'].with_user(SUPERUSER_ID).generate(self.unscoped_key, None, 'Another key without a scope', self.tomorrow)
