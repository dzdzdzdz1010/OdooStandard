...
from odoo.tools import BinaryValue, float_repr, mute_logger
...
%endif"""
    ...
    def test_81_binary_assign(self):
        binary_value1 = BinaryValue.from_bytes(b'content')
        ...
        binary_value2 = BinaryValue.from_bytes(b'test')
    ...
    def test_82_binary_bin_size(self):
        binary_value = BinaryValue.from_bytes(b'content')
        ...
        record = record.with_context(bin_size=True)
        record.binary = binary_value = BinaryValue.from_bytes(b'test')
        record.env.invalidate_all()
        assertBinaryValue(f'{binary_value.size}.00 bytes')
    ...
    def test_85_binary_guess_zip(self):
        ...
        self.env['test_orm.binary_svg'].with_user(self.user_demo).create({
            ...
            'image_wo_attachment': BinaryValue.from_bytes(ZIP_RAW),
        })
    ...
    def test_86_text_base64_guess_svg(self):
        with self.assertRaises(UserError) as e:
            self.env['test_orm.binary_svg'].with_user(self.user_demo).create({
                'name': 'Test without attachment',
                'image_wo_attachment': BinaryValue.from_bytes(SVG_RAW),
            })
    ...
    def test_91_binary_svg_attachment(self):
        ...
        record = self.env['test_orm.binary_svg'].create({
            'name': 'Test without attachment',
            'image_attachment': BinaryValue.from_bytes(SVG_RAW),
        })
    ...
    def test_92_binary_self_avatar_svg(self):
        ...
        demo_user.with_user(demo_user).image_1920 = BinaryValue.from_bytes(SVG_RAW)
    ...
    def test_94_image(self):
        ...
        f.seek(0)
        image_w = BinaryValue.from_bytes(f.read())

        f = io.BytesIO()
        Image.new('RGB', (2000, 4000), '#4169E1').save(f, 'PNG')
        f.seek(0)
        image_h = BinaryValue.from_bytes(f.read())
        ...
        with self.assertRaises(UserError):
            record.write({
                'image': BinaryValue.from_bytes(b'invalid image'),
            })
