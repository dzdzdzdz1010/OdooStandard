...
class TestBinaryExport(TestBasicExport):
    def test_image(self):
        ...
        file = BinaryValue.from_file('test_orm/tests/test_vectors/image')
        file_b64 = base64.b64encode(file.content).decode()
        value = converter.value_to_html(file, {})

        self.assertEqual(
            str(value), f'<img src="data:image/jpg;base64,{file_b64}">')

        file = BinaryValue.from_file('test_orm/tests/test_vectors/pdf')

        with self.assertRaises(ValueError):
            converter.value_to_html(file, {})

        file = BinaryValue.from_file('test_orm/tests/test_vectors/pptx')
