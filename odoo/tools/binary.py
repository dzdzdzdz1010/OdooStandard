...
__all__ = ['BinaryFile', 'BinaryValue']


class BinaryValue(Buffer):
    """Lazy representation of bytes. Abstract class."""
...
class BinaryFile(BinaryValue):
    """Lazy loaded file."""
    __slots__ = ('__content', '__file', '__mimetype', '__path')

    def __init__(self, path: str, *, filter_ext=(), file: io.FileIO | None = None):
        """ Open a file as a binary value.

        :param path: path to the file
        :param filter_ext: passed to `file_open` function
        :param file: if set, use it instead of opening; path must match the name
        :raise OSError: if the file cannot be opened
        """
        if file is None:
            file = file_open(path, 'rb', filter_ext=filter_ext)
            path = file.name  # get the absolute path
        else:
            assert not filter_ext, "filter_ext cannot be used with file"
            assert file.mode == 'rb' and not file.closed and file.seekable(), "file needs to be opened in 'rb' mode"
        assert not path or os.path.isabs(path), "expecting an absolute path"
        # __content is None <=> the file is open
        self.__content: bytes | None = None
        self.__file: io.FileIO = file
        self.__path = path or file.name
    ...
    def __copy__(self):
        if self.__content is None:
            ...
            return BinaryFile(self.__path, file=os.fdopen(os.dup(self.__file.fileno()), 'rb'))
