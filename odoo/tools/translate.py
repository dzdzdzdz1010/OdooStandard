...
def should_translate_default_attr(term, field_value):
    """ Return `False` if the term is found within `field_value` and that it is
        ...
        by a user (see `USER_ONLY_TRANSLATED_ATTRS`).
    ...
    re_attr_no_export = r'(%s)=[\'"]' % '|'.join(USER_ONLY_TRANSLATED_ATTRS) + re.escape(term_unescaped) + r'[\'"]'
        ...
        for record in records.with_context(check_translations=True):
            ...
            for field_name, field in record._fields.items():
                ...
                for term_en, term_langs in translation_dictionary.items():
                    if not should_translate_default_attr(term_en, value_en):
                        continue
